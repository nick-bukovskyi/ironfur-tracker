...
local function RefreshPresentation(now)
  ...
  elseif
    playerState.druid == true
    and playerState.formDisposition == FORM_BEAR
    and (ns.Config.GetAlwaysVisible() or ns.Tracker.HasActiveTicks())
  then
...
local function HandlePlayerSpellcast(spellID)
  ...
  if spellID ~= spellIDs.IRONFUR and spellID ~= spellIDs.MANGLE and spellID ~= spellIDs.FRENZIED_REGENERATION then
    return
  end
...
local function OnUpdate()
  ...
  if
    ...
    and not ns.EditMode.IsPreviewActive()
  then
    ns.Bar.RenderLive(ns.Tracker.GetTicks(), now)
  elseif changed then
...
local function Initialize(eventFrame)
  ...
  ns.Media.Initialize(function(mediaType, name)
    ...
    elseif mediaType == "statusbar" then
      selected = ns.Config.GetTexture("barTexture") == name or ns.Config.GetTexture("backdropTexture") == name
    else
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
  ...
  elseif event == "ACTIVE_TALENT_GROUP_CHANGED" or event == "TRAIT_CONFIG_UPDATED" or event == "SPELLS_CHANGED" then
    ns.Tracker.ClearProcState()
    UpdatePlayerState()
