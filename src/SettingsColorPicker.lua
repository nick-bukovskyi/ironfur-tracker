...
local function ReadColor(key, index)
  if index then
    return ns.Config.GetStackColor(index)
  end
...
local function WriteColor(edit, r, g, b, a)
  if edit.index then
    return ns.Config.SetStackColor(edit.index, r, g, b, a)
  end
...
function Picker.Cancel()
  ...
  if ColorPickerFrame:GetExtraInfo() == edit then
...
function Picker.Open(key, index)
  ...
  if index and not ns.Config.HasStackColor(index) then
    return
  end
  Picker.Cancel()
  if not hideHooked then
    ColorPickerFrame:HookScript("OnHide", function()
      if session and ColorPickerFrame:GetExtraInfo() == session then
        session = nil
      end
    end)
    hideHooked = true
  ...
  local function ApplyPreview()
    if session ~= edit or edit.initializing or not canEdit() or ColorPickerFrame:GetExtraInfo() ~= edit then
      return
    end
  ...
  ColorPickerFrame:SetupColorPickerAndShow({
    ...
    cancelFunc = function()
      if session == edit and ColorPickerFrame:GetExtraInfo() == edit then
        Picker.Cancel()
      end
    end,
  })
