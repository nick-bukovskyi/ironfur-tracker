...
local function HideEyeTooltip()
  ...
  if eyeButton and GameTooltip:GetOwner() == eyeButton then
    GameTooltip:Hide()
  end
end
...
local function RefreshEyeButton()
  ...
  eyeButton:GetNormalTexture():SetTexCoord(left, left + EYE_FRAME_WIDTH, 0, EYE_FRAME_HEIGHT)
...
local function ApplyNumber(row, rawValue)
  if controlsRefreshing or issecretvalue(rawValue) then
    return
  end
...
local function RefreshColor(key)
  ...
  if row then
    row.swatch:SetColorTexture(ns.Config.GetColor(key))
  end
end
...
local function CreateNumericRow(definition)
  ...
  slider:RegisterCallback(MinimalSliderWithSteppersMixin.Event.OnValueChanged, function(_, value)
    if controlsRefreshing or issecretvalue(value) or type(value) ~= "number" then
      return
    end
...
local function CreateColorRow(key, colorMode, label, tooltip)
  ...
  AddColorSwatch(row, row.label, function()
    OpenColorPicker(key)
  end)
  if tooltip then
    local function HideTooltip()
      if GameTooltip:GetOwner() == row.button then
        GameTooltip:Hide()
      end
    end
...
local function CreateTextureRow(key, mediaType)
  ...
  dropdown:SetupMenu(function(_, rootDescription)
    ...
    for _, option in ipairs(ns.Media.GetOptions(mediaType)) do
      ...
      local item = rootDescription:CreateRadio(name, function()
        return ns.Config.GetTexture(key) == name
      end, function()
        if not CanEdit() then
          return
        end
  ...
  dropdown:SetupMenu(function(_, rootDescription)
    for _, option in ipairs(ns.Config.GetChoiceOptions(key)) do
      ...
      rootDescription:CreateRadio(option.label, function()
        return ns.Config.GetChoice(key) == value
      end, function()
        if not CanEdit() then
          return
        end
        ClearInputFocus(false)
        CloseTransientControls()
        if ns.Config.SetChoice(key, value) then
          ApplySettingsChange()
        end
      end)
    end
...
local function StackCountLabel(count, nextCount)
  if not nextCount then
    return tostring(count) .. "+ stacks"
  end
  if nextCount > count + 1 then
    return tostring(count) .. "-" .. tostring(nextCount - 1) .. " stacks"
  end
...
local function ReadStackCount()
  ...
  if not count or count ~= math.floor(count) or count < 1 or count > ns.Config.GetMaximumStackColors() then
    return nil
  end
  return count
end
...
local function OnStackInputChanged()
  if stackColors.updatingInput then
    return
  end
...
local function CreateStackColorRows()
  ...
  AddColorSwatch(stackColors, dropdown, function()
    local count = ReadStackCount()
    if count and ns.Config.HasStackColor(count) then
      OpenColorPicker("stackColor", count)
    end
  end)
  dropdown:SetupMenu(function(_, rootDescription)
    rootDescription:SetScrollMode(300)
    ...
    for index, count in ipairs(thresholds) do
      ...
      local item = rootDescription:CreateRadio(StackCountLabel(count, thresholds[index + 1]), function()
        return selectedStack == stackIndex
      end, function()
        if not CanEdit() then
          return
        end
  ...
  removeButton:SetScript("OnClick", function()
    ...
    local count = ReadStackCount()
    if not count or not ns.Config.HasStackColor(count) then
      return
    end
    CloseTransientControls()
    ClearInputFocus(false)
...
local function CreateFontFamilyRow()
  ...
  dropdown:SetupMenu(function(_, rootDescription)
    ...
    for _, option in ipairs(ns.Media.GetOptions("font")) do
      local name = option.name
      local item = rootDescription:CreateRadio(name, function()
        return ns.Config.GetFontFamily() == name
      end, function()
        if not CanEdit() then
          return
        end
        CloseTransientControls()
        if ns.Config.SetFontFamily(name) then
          ApplySettingsChange()
        end
      end)
      item:AddInitializer(function(button)
...
local function AddNumericSection(section, key)
  for _, definition in ipairs(ns.Config.GetNumericDefinitions()) do
    if definition.section == section and (not key or definition.key == key) then
      CreateNumericRow(definition)
    end
  end
end
...
local function BuildControls()
  ...
  CreateColorRow("durationHighColor", "DURATION", "Over half left", "More than 50% of the duration remains")
  CreateColorRow("durationMediumColor", "DURATION", "Quarter to half left", "More than 25% and up to 50% of the duration remains")
  CreateColorRow("durationLowColor", "DURATION", "Quarter or less left", "25% or less of the duration remains")
...
local function EnsurePanel()
  ...
  scrollFrame:HookScript("OnScrollRangeChanged", function(frame, _, range)
    if frame:GetVerticalScroll() > range then
      frame:SetVerticalScroll(range)
    end
  end)
...
function Settings.Refresh()
  ...
  for key in pairs(colorRows) do
    RefreshColor(key)
  end
  for _, row in pairs(textureRows) do
    RefreshTexture(row)
  end
  for key, row in pairs(choiceRows) do
    ...
    for _, option in ipairs(ns.Config.GetChoiceOptions(key)) do
      if option.value == selected then
        row.dropdown:OverrideText(option.label)
      end
    end
  end
...
function Settings._GetTestState()
  return {
    ...
    colorSession = ns.SettingsColorPicker._GetSession(),
    eyeButton = eyeButton,
    choices = choiceRows,
    ...
    stackColors = stackColors and {
      dropdown = stackColors.dropdown,
      button = stackColors.button,
      swatch = stackColors.swatch,
      addButton = stackColors.addButton,
      removeButton = stackColors.removeButton,
      selectedIndex = selectedStack,
      input = stackColors.input,
      hint = stackColors.hint,
    },
  }
end
