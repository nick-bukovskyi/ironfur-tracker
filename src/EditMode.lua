...
local function EnsureSelection()
  ...
  if
    not MinimalSliderWithSteppersMixin
    or not MinimalSliderWithSteppersMixin.Event
    or not MinimalSliderWithSteppersMixin.Event.OnValueChanged
  then
    return false
  end
...
function EditMode.TryAttachOverlayToggle()
  ...
  if
    not library
    or minor ~= SUPPORTED_ENHANCE_EDIT_MODE_MINOR
...
function EditMode.TryAttachManager()
  ...
  if
    type(manager.SelectSystem) ~= "function"
