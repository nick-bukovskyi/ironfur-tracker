...
local function NormalizeColor(color, defaults)
    if issecretvalue(color) or type(color) ~= "table" then color = {} end
...
local function IsChoice(key, value)
    if issecretvalue(value) or type(value) ~= "string" then return false end
...
local function NormalizeStackColors(bar, reset, legacyDense)
    ...
    if reset or issecretvalue(colors) or type(colors) ~= "table" then
        ...
        for index = 1, MAXIMUM_STACK_COLORS do
            if issecretvalue(colors[index]) or colors[index] ~= nil then highest = index end
        end
...
local function NormalizeAppearance(bar, reset, legacyDense)
    ...
    for key, options in pairs(CHOICES) do
        if reset or not IsChoice(key, bar[key]) then bar[key] = options[1].value end
    end
...
function Config.SetFontFamily(name)
    if not database or not IsMediaName(name) or name == "Default" then return false end
...
function Config.SetChoice(key, value)
    if not database or not IsChoice(key, value) then return false end
...
function Config.GetStackColorCount()
    if not database then return 0 end
    local count = 0
    for index = 1, MAXIMUM_STACK_COLORS do
        if database.bar.stackColors[index] then count = count + 1 end
    end
    return count
end

local function StackColor(index)
    if not database or not IsFiniteNumber(index) or index ~= math.floor(index)
        or index < 1 or index > MAXIMUM_STACK_COLORS then return nil end
...
function Config.GetStackThresholds()
    ...
    if database then
        for index = 1, MAXIMUM_STACK_COLORS do
            if database.bar.stackColors[index] then thresholds[#thresholds + 1] = index end
        end
    end
    return thresholds
end
...
function Config.SetStackColor(index, r, g, b, a)
    local color = StackColor(index)
    if not color or not IsFiniteNumber(r) or r < 0 or r > 1
        or not IsFiniteNumber(g) or g < 0 or g > 1
        or not IsFiniteNumber(b) or b < 0 or b > 1
        or not IsFiniteNumber(a) or a < 0 or a > 1 then return false end
    color.r, color.g, color.b, color.a = r, g, b, a
    return true
end
...
function Config.AddStackColor(index)
    if not database or not IsFiniteNumber(index) or index ~= math.floor(index)
        or index < 1 or index > MAXIMUM_STACK_COLORS or StackColor(index) then return false end
    database.bar.stackColors[index] = NormalizeColor(nil, InheritedStackColor(index))
    return index
end

function Config.RemoveStackColor(index)
    if not StackColor(index) then return false end
...
function Config.GetBarColor(stackCount, progress)
    ...
    elseif mode == "STACKS" and IsFiniteNumber(stackCount)
