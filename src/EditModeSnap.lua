...
local GEOMETRY_METHODS = {
    "GetScaledSelectionSides", "GetScaledSelectionCenter", "GetScaledCenter",
    "GetLeftOffset", "GetRightOffset", "GetTopOffset", "GetBottomOffset",
    "GetSelectionOffset", "GetCombinedSelectionOffset", "GetCombinedCenterOffset",
    "GetSnapOffsets",
}
local PARENT_COORDINATES = {
    "topLevelParentCenterX", "topLevelParentCenterY",
    "topLevelParentLeft", "topLevelParentRight", "topLevelParentBottom", "topLevelParentTop",
    "topLevelParentWidth", "topLevelParentHeight",
}
local ANCHORS = {
...
local function HasReadableRect(frame)
    ...
    return IsNumber(left) and IsNumber(bottom) and IsNumber(width) and IsNumber(height)
        and width > 0 and height > 0 and IsNumber(centerX) and IsNumber(centerY)
end
...
function mover:GetFrameMagneticEligibility(candidate)
    if candidate == self.frame
        ...
        or not HasReadableGeometry(candidate, candidate.Selection) then
        return false, false
    end
...
local function GetSnapInfos()
    ...
    if not active or InCombatLockdown() or not manager or not magnetism or not mixin
        or type(manager.IsEditModeActive) ~= "function" or type(manager.IsSnapEnabled) ~= "function"
        or not manager:IsEditModeActive() or not manager:IsSnapEnabled()
        ...
        or not HasReadableRect(UIParent) then
        return nil
    end
