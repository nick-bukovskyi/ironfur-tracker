...
function getfenv(level, ...)
  ...
  if level ~= 0 then
    error("only getfenv(0) is exercised by the bundled libraries", 2)
  end
  return _G
end
...
C_UIFileAsset = {
  IsKnownFile = function(asset, ...)
    ...
    if known == nil then
      error("file asset has no test fixture: " .. tostring(asset), 2)
    end
    return known
  end,
}
...
C_ClassColor = {
  GetClassColor = function(className, ...)
    ...
    if className ~= "DRUID" then
      error("unexpected class-color fixture", 2)
    end
    if _G._stubClassColorAvailable then
      return druidClassColor
    end
  end,
}
...
function TextureMixin:SetAllPoints(...)
  self._allPoints = { ... }
end
function TextureMixin:SetColorTexture(...)
  self._color = { ... }
end
...
function TextureMixin:SetVertexColor(r, g, b, a, ...)
  ...
  if a ~= nil then
    AssertType(a, "number", "alpha")
  end
  self._vertexColor = { r, g, b, a or 1 }
end
function TextureMixin:SetSize(width, height)
  self._width = width
  self._height = height
end
function TextureMixin:SetWidth(width)
  self._width = width
end
function TextureMixin:SetHeight(height)
  self._height = height
end
function TextureMixin:GetWidth()
  return self._width or 0
end
function TextureMixin:GetHeight()
  return self._height or 0
end
function TextureMixin:SetPoint(...)
  self._point = { ... }
end
...
function TextureMixin:ClearAllPoints()
  self._point = nil
end
function TextureMixin:SetSnapToPixelGrid(value)
  self._snapToPixelGrid = value
end
function TextureMixin:SetTexelSnappingBias(value)
  self._texelSnappingBias = value
end
function TextureMixin:Show()
  self._shown = true
end
function TextureMixin:Hide()
  self._shown = false
end
function TextureMixin:IsShown()
...
function FontStringMixin:SetPoint(...)
  self._point = { ... }
end
function FontStringMixin:ClearAllPoints(...)
  AssertNoExtraArguments("FontString:ClearAllPoints", ...)
  self._point = nil
end
...
function FontStringMixin:SetTextColor(...)
  self._color = { ... }
end
function FontStringMixin:SetText(value)
  self._text = tostring(value or "")
end
function FontStringMixin:GetText()
  return self._text or ""
end
function FontStringMixin:SetWidth(width)
  self._width = width
end
function FontStringMixin:SetHeight(height)
  self._height = height
end
function FontStringMixin:SetJustifyH(justify)
  self._justifyH = justify
end
function FontStringMixin:Show()
  self._shown = true
end
function FontStringMixin:Hide()
  self._shown = false
end
function FontStringMixin:IsShown()
  return self._shown == true
end
...
function FontStringMixin:GetFont(...)
  ...
  if self._fontObject and not self._fontFile then
...
function FontStringMixin:SetFont(asset, height, flags, ...)
  ...
  if self._menuOwned then
    error("Blizzard menu compositor forbids FontString:SetFont", 2)
  end
  if type(asset) ~= "string" and type(asset) ~= "number" then
    error("SetFont requires a font asset", 2)
  end
  ...
  if success == nil then
    error("font asset has no test fixture: " .. tostring(asset), 2)
  end
  if success then
    self._fontFile, self._fontHeight, self._fontFlags = asset, height, flags or ""
  end
  return success
end
function FontStringMixin:GetShadowColor(...)
  AssertNoExtraArguments("GetShadowColor", ...)
  if self._fontObject and not self._shadowColor then
    return self._fontObject:GetShadowColor()
  end
...
function FontStringMixin:SetShadowColor(r, g, b, a, ...)
  ...
  if a ~= nil then
    AssertType(a, "number", "shadow alpha")
  end
  self._shadowColor = { r, g, b, a or 1 }
end
function FontStringMixin:GetShadowOffset(...)
  AssertNoExtraArguments("GetShadowOffset", ...)
  if self._fontObject and not self._shadowOffset then
    return self._fontObject:GetShadowOffset()
  end
...
function CreateFont(name, ...)
  ...
  if name == "" or _G[name] ~= nil then
    error("font object name must be new and nonempty", 2)
  end
...
function FontStringMixin:SetFontObject(font, ...)
  ...
  if getmetatable(font) ~= FontObjectMixin then
    error("SetFontObject requires a known Font object", 2)
  end
...
function FrameMixin:SetWidth(width)
  self._width = width
end
function FrameMixin:SetHeight(height)
  self._height = height
end
...
function FrameMixin:GetSize()
  return self:GetWidth(), self:GetHeight()
end
function FrameMixin:GetScale()
  return self._scale or 1
end
function FrameMixin:SetScale(value)
  self._scale = value
end
...
function FrameMixin:GetEffectiveScale()
  if self._effectiveScale ~= nil then
    return self._effectiveScale
  end
...
function FrameMixin:SetPoint(...)
  ...
  if not replaced then
    self._points[#self._points + 1] = values
  end
  ...
  if point == "CENTER" and relativePoint == "CENTER" and relativeTo and relativeTo.GetCenter then
...
function FrameMixin:GetNumPoints()
...
function FrameMixin:GetRect()
  ...
  if self._rectOverride then
    return unpackValues(self._rectOverride)
  end
...
function FrameMixin:SetFrameStrata(strata)
  self._frameStrata = strata
end
function FrameMixin:SetFrameLevel(level)
  self._frameLevel = level
end
function FrameMixin:GetFrameLevel()
  return self._frameLevel or 0
end
function FrameMixin:SetMinMaxValues(minimum, maximum)
  self._min = minimum
  self._max = maximum
end
function FrameMixin:SetStatusBarTexture(texture)
  self._statusBarTexture = texture
end
function FrameMixin:SetStatusBarColor(...)
...
function FrameMixin:SetBackdrop(info, ...)
  ...
  if info ~= nil then
    ...
    for key in pairs(info) do
      if key ~= "edgeFile" and key ~= "edgeSize" and key ~= "bgFile" and key ~= "tile" and key ~= "tileSize" and key ~= "insets" then
    ...
    if info.edgeSize ~= nil then
      AssertType(info.edgeSize, "number", "backdrop edge size")
    end
  end
  self._backdrop = info
end
...
function FrameMixin:SetChecked(checked, ...)
  ...
  if self._type ~= "CheckButton" then
    error("SetChecked requires a CheckButton", 2)
  end
  if checked ~= nil then
    AssertType(checked, "boolean", "checked")
  end
...
function FrameMixin:GetChecked(...)
  ...
  if self._type ~= "CheckButton" then
    error("GetChecked requires a CheckButton", 2)
  end
...
function FrameMixin:SetScrollChild(child, ...)
  ...
  if self._type ~= "ScrollFrame" then
    error("SetScrollChild requires a ScrollFrame", 2)
  end
  AssertType(child, "table", "scroll child")
  if not child._type then
    error("scroll child must be a frame", 2)
  end
  child._parent = self
...
function FrameMixin:GetVerticalScroll(...)
  ...
  if self._type ~= "ScrollFrame" then
    error("GetVerticalScroll requires a ScrollFrame", 2)
  end
...
function FrameMixin:GetVerticalScrollRange(...)
  ...
  if self._type ~= "ScrollFrame" then
    error("GetVerticalScrollRange requires a ScrollFrame", 2)
  end
...
function FrameMixin:SetVerticalScroll(offset, ...)
  ...
  if self._type ~= "ScrollFrame" then
    error("SetVerticalScroll requires a ScrollFrame", 2)
  end
...
function FrameMixin:GetValue(...)
  ...
  if self._type ~= "StatusBar" then
    error("GetValue requires a StatusBar", 2)
  end
...
function FrameMixin:IsShown()
  return self._shown == true
end
function FrameMixin:IsVisible()
  if self._visibleOverride ~= nil then
    return self._visibleOverride
  end
  return self:IsShown()
end
...
function FrameMixin:CreateTexture(name, layer, template, subLevel)
  if name ~= nil then
    AssertType(name, "string", "texture name")
  end
  if layer ~= nil then
    AssertType(layer, "string", "texture layer")
  end
...
function FrameMixin:CreateLine(name, layer, template, ...)
  ...
  if name ~= nil then
    AssertType(name, "string", "line name")
  end
  if layer ~= nil then
    AssertType(layer, "string", "line layer")
  end
...
function FrameMixin:CreateFontString(name, layer, template)
  if name ~= nil then
    AssertType(name, "string", "font string name")
  end
  if layer ~= nil then
    AssertType(layer, "string", "font string layer")
  end
...
function FrameMixin:SetMovable(value)
  self._movable = value and true or false
end
function FrameMixin:IsMovable()
  return self._movable == true
end
function FrameMixin:SetClampedToScreen(value)
  self._clampedToScreen = value and true or false
end
function FrameMixin:EnableMouse(value)
  self._mouseEnabled = value and true or false
end
...
function FrameMixin:StopMovingOrSizing()
  self._moving = false
end
function FrameMixin:SetUserPlaced(value)
  self._userPlaced = value and true or false
end
...
function FrameMixin:SetEnabled(enabled, ...)
  ...
  if self._type ~= "Button" then
    error("SetEnabled fixture requires a Button", 2)
  end
  AssertType(enabled, "boolean", "button enabled")
  self._enabled = enabled
end
function FrameMixin:GetText()
  return self._text or ""
end
function FrameMixin:SetNumber(value)
  self:SetText(value)
end
function FrameMixin:GetNumber()
  return tonumber(self:GetText()) or 0
end
function FrameMixin:SetAutoFocus(value)
  self._autoFocus = value and true or false
end
function FrameMixin:SetNumeric(value)
  self._numeric = value and true or false
end
function FrameMixin:SetMaxLetters(value)
  self._maxLetters = value
end
function FrameMixin:SetJustifyH(value)
  self._justifyH = value
end
function FrameMixin:SetNormalTexture(asset, ...)
  AssertNoExtraArguments("SetNormalTexture", ...)
  if self._type ~= "Button" then
    error("SetNormalTexture requires a Button", 2)
  end
  if type(asset) ~= "string" and type(asset) ~= "number" then
...
function FrameMixin:GetNormalTexture(...)
  ...
  if self._type ~= "Button" then
    error("GetNormalTexture requires a Button", 2)
  end
  return self._normalTexture
end
function FrameMixin:SetFocus()
  self._hasFocus = true
end
function FrameMixin:HasFocus()
...
function FrameMixin:Init(value, minimum, maximum, steps, formatters)
  ...
  if type(value) ~= "number" or type(minimum) ~= "number" or type(maximum) ~= "number" or type(steps) ~= "number" or steps <= 0 then
...
function FrameMixin:SetSystem(system)
  self._system = system
end
function FrameMixin:ShowHighlighted()
  self._selectionState = "highlighted"
  self:Show()
end
function FrameMixin:ShowSelected()
  self._selectionState = "selected"
  self:Show()
end
...
function CreateFrame(frameType, name, parent, template, ...)
  ...
  if name ~= nil then
    AssertType(name, "string", "frame name")
  end
...
function FrameMixin:CloseMenu(...)
  ...
  for _, row in ipairs(self._menuRows or {}) do
    row:Hide()
  end
end
    ...
    if not row then
      ...
      function row:AttachTemplate(template, ...)
        ...
        if template ~= "BackdropTemplate" then
          error("unexpected menu attachment template", 2)
        end
...
function GameTooltip:SetOwner(owner, anchor, ...)
  ...
  if type(owner) ~= "table" or not owner._type then
    error("tooltip owner must be a frame", 2)
  end
  if anchor ~= "ANCHOR_RIGHT" then
    error("unexpected tooltip anchor: " .. tostring(anchor), 2)
  end
...
function ColorPickerFrame:SetupColorPickerAndShow(info, ...)
  ...
  for key in pairs(info) do
    if not fields[key] then
      error("unexpected color picker field: " .. tostring(key), 2)
    end
  end
  for _, channel in ipairs({ "r", "g", "b" }) do
    AssertType(info[channel], "number", channel)
  end
  AssertType(info.swatchFunc, "function", "color picker swatch callback")
  if info.hasOpacity ~= nil then
    AssertType(info.hasOpacity, "boolean", "color picker opacity toggle")
  end
  if info.opacity ~= nil then
    AssertType(info.opacity, "number", "color picker opacity")
  end
  if info.opacityFunc ~= nil then
    AssertType(info.opacityFunc, "function", "color picker opacity callback")
  end
  if info.cancelFunc ~= nil then
    AssertType(info.cancelFunc, "function", "color picker cancel callback")
  end
  self._info = info
...
function _G._SetPickerColor(r, g, b, a)
  if not ColorPickerFrame:IsShown() then
    error("color picker is not shown", 2)
  end
...
function EditModeManagerFrame:IsEditModeActive()
  return self._editModeActive == true
end
function EditModeManagerFrame:ShowSystemSelections()
  self._selectionsShown = true
end
function EditModeManagerFrame:HideSystemSelections()
  self._selectionsShown = false
end
function EditModeManagerFrame:SelectSystem(system)
  self._selectedSystem = system
end
...
function EditModeManagerFrame:IsSnapEnabled()
  return self._snapEnabled == true
end
function EditModeManagerFrame:SetEnableSnap(value)
  self._snapEnabled = value and true or false
end
...
function EditModeMagnetismManager:GetMagneticFrameInfos(mover, ...)
  ...
  if not _G._stubMagneticFrameInfos then
    return nil
  end
  ...
  for _, info in ipairs(_G._stubMagneticFrameInfos) do
    if info.frame == UIParent then
      ...
      if (info.isCornerSnap and (horizontal or vertical)) or (info.isHorizontal and horizontal) or (not info.isHorizontal and vertical) then
        accepted[#accepted + 1] = info
      end
    end
  end
...
function EditModeMagnetismManager:GetPreviewLineAnchors(info, ...)
  ...
  for _, item in ipairs({ { "TOP", "Top" }, { "BOTTOM", "Bottom" }, { "LEFT", "Left" }, { "RIGHT", "Right" } }) do
    if relativePoint:find(item[1], 1, true) then
      anchors[#anchors + 1] = item[2]
    end
  end
  return anchors
end
...
function _G._CreateMagneticFrame(centerX, centerY, width, height, scale)
  ...
  for key, method in pairs(EditModeSystemMixin) do
    frame[key] = method
  end
  return frame
end
...
function UnitClass(unit, ...)
  ...
  if unit ~= "player" then
    error("UnitClass expected player", 2)
  end
