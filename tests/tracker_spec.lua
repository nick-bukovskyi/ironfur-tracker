...
local function HookCount(methodName)
  local hooks = EditModeManagerFrame._secureHooks and EditModeManagerFrame._secureHooks[methodName]
...
describe("Loading, persistence, and lifecycle", function()
  ...
  it("recovers only invalid SavedVariables fields", function()
    ...
    expect(normalized.schemaVersion).to_equal(11)
...
local function GridSnapInfos(offsetX, offsetY)
  return {
    { frame = UIParent, point = "CENTER", relativePoint = "CENTER", distance = 1, offset = offsetX, isHorizontal = true },
    { frame = UIParent, point = "CENTER", relativePoint = "CENTER", distance = 1, offset = offsetY, isHorizontal = false },
  }
end
...
describe("Placement-only Edit Mode snapping", function()
  ...
  it("converts controlled native-element output to an independent screen position", function()
    ...
    local info = { frame = target, point = "LEFT", relativePoint = "RIGHT", distance = 5, offset = 0, isHorizontal = true }
  ...
  it("uses current bar size and scaled native geometry without losing fractions", function()
    ...
    local info = { frame = target, point = "RIGHT", relativePoint = "LEFT", distance = 5, offset = 0, isHorizontal = true }
  ...
  local cancellationCases = {
    ...
    {
      name = "hidden native selections",
      run = function()
        EditModeManagerFrame:HideSystemSelections()
      end,
    },
    {
      name = "selection hiding",
