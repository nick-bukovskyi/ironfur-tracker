...
local function CreateExternalEye(hidden)
    ...
    button:SetScript("OnClick", function(self, mouseButton)
        if mouseButton ~= "LeftButton" then error("external eye fixture expects LeftButton") end
...
local function CreateExternalLibrary(minor, hidden, withEye)
    ...
    if withEye then library.internal.managerEyeButton = CreateExternalEye(hidden) end
    ...
    function library:AddFrame(frame, callback, defaults, ...)
        if select("#", ...) ~= 0 or type(frame) ~= "table" or not frame._type
            or (callback ~= nil and type(callback) ~= "function")
            or (defaults ~= nil and type(defaults) ~= "table") then
...
local function TriggerExternalEnter()
    ...
    if eye then eye.allHidden = false end
    for _, callback in ipairs(externalLibrary._fixtureEnterCallbacks) do callback() end
end
